...
```bash
bash scripts/check_dataset_mats.sh --dataroot ./Dataset
```

如果还要检查低比例实验所需文件：

```bash
bash scripts/check_dataset_mats.sh --dataroot ./Dataset --check-split
```
...
当前仓库中的 DFG 入口脚本已经默认打开第一阶段 REGZSL 关系迁移配置：

- 保留 ZeroDiff 的 critic 循环
- 每个 critic step 抽一个 episode
- 默认 `rel_n_way=3`、`rel_k_shot=4`
- 每个 episode 的样本数固定为 `24`
- 默认同时接入 `RKdDistance` 和 `RKdAngle`
- 关系损失内部默认权重为 `rel_dist_ratio=1`、`rel_angle_ratio=2`
- 每个 epoch 的更新次数按原 `batch_size=64` 的量级做了归一化
