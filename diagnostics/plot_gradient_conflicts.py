"""Plot gradient conflict and base-anchor diagnostics."""
...
def main() -> None:
    ...
    for row in rows:
        for name in ("class", "instance"):
            key = f"{name}_opposing_component_ratio"
            if key not in row:
                row[key] = str(max(0.0, -float(row[f"cos_{name}_base"])))
    ...
    plot_summary(
        axes[1],
        summaries["conflict_class_instance"],
        "Class vs. instance topology",
        "#9467bd",
    )
